...
public class Equation {
  ...
  public void calcularRaices() {
    ...
    switch (getCaso()) {
      case '1':
        ...
        Fraccion suma =
            new Fraccion(
                (raices[0].getNumerador() + raices[1].getNumerador()), raices[0].getDenominador());
        Fraccion resta =
            new Fraccion(
                (raices[0].getNumerador() - raices[1].getNumerador()), raices[0].getDenominador());
        suma.simplificar();
    ...
    if (this.caso == '1') {
      ...
      if (isDoubleEntero(B.getNumerador())) {
        if (A.isFraccionEntera() && B.isFraccionEntera()) {
          raicesS[0] =
              toEntero(
                  dividir(A.getNumerador(), A.getDenominador())
                      + dividir(B.getNumerador(), B.getDenominador()));
          raicesS[1] =
              toEntero(
                  dividir(A.getNumerador(), A.getDenominador())
                      - dividir(B.getNumerador(), B.getDenominador()));
      ...
      case '2':
        r =
            "r=\\dfrac{-\\left("
                ...
                + toEntero(a)
                + ")}\\quad r="
                + getRaices(0).toString()
                + "\\pm"
                + getRaices(1).toString()
                + "i \\Rightarrow \\lambda="
                + raicesS[0]
                + ",\\mu="
                + raicesS[1];
  ...
  public void calcularPVI() {
    ...
    if (this.caso == '1') {
      double r1 =
          dividir(this.raicesReducidas[0].getNumerador(), this.raicesReducidas[0].getDenominador());
      double r2 =
          dividir(this.raicesReducidas[1].getNumerador(), this.raicesReducidas[1].getDenominador());
      ...
      sistema[0][2] = this.pv1[1];
      D =
          (r1 * Math.pow(Math.E, r1 * this.pv1[0]) * Math.cos(r2 * this.pv1[0]))
              - (r2 * Math.pow(Math.E, r1 * this.pv1[0]) * Math.sin(r2 * this.pv1[0]));
      E =
          (r1 * Math.pow(Math.E, r1 * this.pv1[0]) * Math.sin(r2 * this.pv1[0]))
              + (r2 * Math.pow(Math.E, r1 * this.pv1[0]) * Math.cos(r2 * this.pv1[0]));
      sistema[1][0] = D;
      sistema[1][1] = E;
      sistema[1][2] = this.pv2[1];
      detg = hallarDet(sistema[0][0], sistema[1][1], sistema[1][0], sistema[0][1]);
